...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, filename=''):
        # input is either filename or x_pixels, y_pixels, and num_channels
        self.input_path = 'input/' #in input folder
        self.output_path = 'output/'  #in output folder
        if x_pixels and y_pixels and num_channels: #then initialize
            self.x_pixels = x_pixels
            self.y_pixels = y_pixels
            self.num_channels = num_channels
            self.array = np.zeros((x_pixels, y_pixels, num_channels)) #num channels is the number of channels... either r/g/b or its combinations
        elif filename: #read the image from file
            self.array = self.read_image(filename)
            self.x_pixels, self.y_pixels, self.num_channels = self.array.shape
        else:
            raise ValueError("Provide proper input i.e, either a filename or specify the dimensions of the image")
    ...
    def read_image(self, filename, gamma=2.2):
        ...
        resized_image = np.vstack(list(im[2]))# stack arrays in sequence vertically (row wise)
        resized_image.resize(im[1], im[0], 3)
        resized_image = resized_image ** gamma#gamma is a value used to decode
...
if __name__ == '__main__':
    ...
    im.write_image('test.png')#there wont be any changes here as we haven't transformed anything